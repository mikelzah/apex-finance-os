...
function emptyState() {
  return {
    ...
    settings: {
      ...
      spendMonths: 3,
      // Окно кривой капитала на главной, в месяцах; null — всё время.
      // Полугодие по умолчанию: месяц шумит одним взносом, год сглаживает
      // до прямой всё, что происходило внутри.
      capitalWindow: 6,
...
/**
 * Убрать снимок за день.
 *
 * Нужно, когда актив ушёл из приложения задним числом: снимок продолжает
 * помнить капитал вместе с ним, и график падает отвесной стеной там, где
 * на самом деле ничего не происходило. Пересчитать старый снимок нельзя —
 * он и есть единственная память о том дне, — поэтому остаётся удалить.
 *
 * Снимок за сегодня удалять бессмысленно: он запишется заново при первом же
 * открытии. Об этом предупреждает экран, а не эта функция.
 */
export async function removeSnapshot(day) {
  let removed = false;
  await mutate((draft) => {
    const before = draft.netWorth.length;
    draft.netWorth = draft.netWorth.filter((r) => r.date !== day);
    removed = draft.netWorth.length < before;
  });
  return removed;
}
