...
(async () => {
  ...
  //
  // Жмём из страницы, а не page.click: тот сперва подкручивает экран к цели,
  // чтобы она оказалась на виду. Переключатель стоит в самом верху, экран
  // от этого уезжает на ноль ещё до нажатия — и проверка ловила бы не потерю
  // положения при перерисовке, а собственную подкрутку.
  await page.evaluate(() => {
    [...document.querySelectorAll('.segmented .segment')]
      .find((s) => /Полгода/.test(s.textContent))
      .click();
  });
  await page.waitForTimeout(600);
  const after = await y();
