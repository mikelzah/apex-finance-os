...
// «Выписка 2026-07-01T14:05:59.534+0300.xlsx». В репозитории такой файл
// не лежит, и создавать его на месте тоже нельзя: в Windows двоеточие
// в имени запрещено, и copyFileSync падает с ENOENT ещё до первой проверки.
//
// Самому имени физический файл и не нужен. Приложение разбирает его строкой
// — расширение и подпись счёта, — и это проверяется отдельно, ниже по тексту.
// Здесь же от копии нужны только байты, а они те же самые.
const FILE = path.join(__dirname, '.out', 'vypiska-alfa.xlsx');
...
(async () => {
  ...
  await p.waitForTimeout(700);
  // Кнопок с этой подписью на пустом экране две: в шапке — она спрашивает,
  // откуда брать (выписка или снимок), — и в самой карточке, которая сразу
  // открывает выбор файла. Нужна вторая: первая открывает шторку, файлового
  // диалога не появляется, и ожидание события висит до таймаута.
  const chooser = p.waitForEvent('filechooser');
  await p.locator('.card .btn:has-text("Загрузить выписку")').first().click();
